import datetime
import time
import requests
from loguru import logger
import json
import urllib.parse

# 节日锚点
holiday_list = [
    # {"周末": "2022-03-12"},
    # {"情人节": "2022-03-12"},
    # {"清明节假期": "2022-04-03"},
    {"劳动节假期": "2022-04-30"},
    {"端午节假期": "2022-06-03"},
    {"中秋节假期": "2022-09-10"},
    {"国庆节节假期": "2022-10-01"},
    {"元旦节假期": "2022-10-01"},
    {"春节假期": "2023-01-21"},
]
# 摸鱼宝典
fish_list = {}


def get_holiday():
    global holiday_list
    """
    获取配置中的节日设置
    :return: list——>[{'节日名':'节日日期'}]
    """
    holiday_content = ''
    # 今天日期
    now_str = datetime.datetime.now().strftime('%Y-%m-%d')
    now = datetime.datetime.strptime(now_str, "%Y-%m-%d")
    print(now_str)
    print(now)
    for holiday_info in holiday_list:
        holiday_name = list(holiday_info.keys())[0]
        holiday_date = holiday_info[holiday_name]
        future = datetime.datetime.strptime(holiday_date, "%Y-%m-%d")
        days = (future - now).days
        holiday_content = holiday_content + '距离' + holiday_name + '还有' + str(days) + '天' + '\n'
    return '距离周末还有' + str(5-datetime.datetime.now().weekday()) + '天' + '\n' + holiday_content


def get_tg():
    """
    获取日记
    :return: bool or str
    """
    url = f"https://fabiaoqing.com/jichou/randomrj.html"
    try:
        res = requests.post(url=url).json()
        return res['tgrj'] + '\n'
    except:
        return False


def get_weather():
    """
    获取天气预报
    :return: str or false
    """
    url = f"http://apis.juhe.cn/simpleWeather/query"
    params = {
        'city': '杭州',
        'key': '7612ddda2313a41481327cbef5261b46',
    }
    try:
        res = requests.get(url=url, params=params).json()
        now_str = datetime.datetime.now().strftime('%Y-%m-%d')
        mytime = time.localtime()
        if mytime.tm_hour < 6:
            hello = '凌晨好！'
        elif mytime.tm_hour < 12:
            hello = '上午好！'
        elif mytime.tm_hour < 18:
            hello = '下午好！'
        else:
            hello = '晚上好！'
        weather_content = f"""【摸鱼办公室】\n今天是 {now_str} 星期 {datetime.datetime.now().weekday() + 1}\n{res['result']['city']} 当前天气 {res['result']['realtime']['info']} {res['result']['realtime']['temperature']}摄氏度\n{hello}摸鱼人！上班点到了，收拾收拾，该吃饭吃饭，该溜达溜达，该上厕所上厕所。别闲着\n"""
        return weather_content
    except:
        return False
def get_weibo():
    headers = {
        "user-agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/100.0.4896.75 Safari/537.36 Edg/100.0.1185.39",
    }

    url = 'https://weibo.com/ajax/statuses/topic_band?sid=v_weibopro&category=all&page=1&count=10'

    resp = requests.get(url, headers=headers)
    dic = resp.json()
    topic = ''

    for i in range(10):
        url = "https://s.weibo.com/weibo?q=%23" + urllib.parse.quote(dic['data']['statuses'][i]['topic']) + "%23"
        a = dic['data']['statuses'][i]['topic']
        topic += f"""{a}:{url}'\n"""

    url = 'https://weibo.com/ajax/statuses/hot_band'

    resp = requests.get(url, headers=headers)
    dic = resp.json()
    hot = ''
    for i in range(10):
        url = "https://s.weibo.com/weibo?q=%23" + urllib.parse.quote(dic['data']['band_list'][i]['note']) + "%23"
        a = dic['data']['band_list'][i]['note']
        hot += f"""{a}:{url}'\n"""
    weibo_content = f"""----------------话题榜-----------------\n{topic}----------------热搜榜-----------------\n{hot}"""
    return weibo_content

if __name__ == '__main__':
    holiday_content = get_holiday()
    weibo_contend = get_weibo()
    if not holiday_content:
        logger.error(f"节日为空。")
        holiday_content = ''
    else:
        logger.info(f"获取到节日：\n{holiday_content}")
    tg_content = get_tg()
    if not tg_content:
        logger.error(f"日记为空。")
        tg_content = ''
    else:
        logger.info(f"获取到日记：\n{tg_content}")
    weather_content = get_weather()
    if not weather_content:
        logger.error(f"天气为空。")
        weather_content = ''
    else:
        logger.info(f"获取到天气：\n{weather_content}")
    complete_content = weather_content + '\n' + holiday_content + '\n' + tg_content + '\n' + '工作再累 一定不要忘记摸鱼哦！有事没事起身去茶水间去厕所去廊道走走，别老在工位上坐着，钱是老板的，但命是自己的！' + '\n看下今天的微博热搜吧：\n' + weibo_contend
    logger.info(f"整合内容开始推送：\n{complete_content}")